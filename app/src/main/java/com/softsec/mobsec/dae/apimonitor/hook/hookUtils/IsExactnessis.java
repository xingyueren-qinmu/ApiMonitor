    ...
    public boolean isChain(Class mClass, Class interceptorClass) {
        ...
        try {
            if (mClass.getName().contains(interceptorClass.getName())) {
                XposedBridge.log("当前class的名字 " + mClass.getName());
            }
...
//                    XposedBridge.log("StringCount "+StringCount+" httpUrlCount "+httpUrlCount+" HeadCount "+HeadCount);
//                }
...
//        static final String FRAGMENT_ENCODE_SET_URI = " \"#<>\\^`{|}";
        ...
        try {
            if (Modifier.isFinal(mClass.getModifiers())
                ...
                if (declaredFields.length >= 3) {
                    for (Field field : declaredFields) {
                        // XposedBridge.log("当前的类名 是  "+Mclass.getName());
                        if (Modifier.isFinal(field.getModifiers())
                                && Modifier.isStatic(field.getModifiers())
    ...
    //                    XposedBridge.log("StaticCount  " + StaticCount + "LongConut   " +
...
//                        XposedBridge.log("字段类型 "+field.getType()+
//                                " 名字 "+field.getName()+
...
//                if (mClass.getName().equals("okio.GzipSource")) {
//                    XposedBridge.log("byteCount " + byteCount + "  CRC32Count  " + CRC32Count + " InflaterCount " + InflaterCount);
//                    XposedBridge.log("总共字段个数 " + declaredFields.length);
//                }
